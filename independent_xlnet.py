...
class CorefModel(object):
  def __init__(self, config, FLAGS):
    ...
    self.FLAGS = FLAGS
    self.xlnet_config = xlnet.XLNetConfig(json_path=FLAGS.xlnet_config_file)
    ...
    self.train_op, learning_rate, _ = model_utils.get_train_op(FLAGS, self.loss)
  ...
  def tensorize_example(self, example, is_training):
    ...
    gold_mentions = sorted(tuple(m) for m in util_xlnet.flatten(clusters))
    ...
    speaker_dict = self.get_speaker_dict(util_xlnet.flatten(speakers))
    sentence_map = example['sentence_map']

    max_sentence_length = self.max_segment_len
    text_len = np.array([len(s) for s in sentences])

    input_ids, seg_ids, input_mask, speaker_ids = [], [], [], []
    for i, (sentence, speaker) in enumerate(zip(sentences, speakers)):
      sent_input_ids = [self.tokenizer.PieceToId(x) for x in sentence]
      sent_input_ids[-2] = SEP_ID
      sent_input_ids[-1] = CLS_ID

      sent_input_mask = [0] * len(sent_input_ids) # 0 for real tokens
      sent_speaker_ids = [speaker_dict.get(s, 3) for s in speaker]
      sent_seg_ids = [SEG_ID_A] * len(sent_input_ids)
      sent_seg_ids[-1] = SEG_ID_CLS # for CLS

      # Zero-pad up to the sequence length.
      if len(sent_input_ids) < max_sentence_length:
          delta_len = max_sentence_length - len(sent_input_ids)
          sent_input_ids = [0] * delta_len + sent_input_ids
          sent_input_mask = [1] * delta_len + sent_input_mask
          sent_speaker_ids = [0] * delta_len + sent_speaker_ids
          sent_seg_ids = [SEG_ID_PAD] * delta_len + sent_seg_ids
      input_ids.append(sent_input_ids)
      speaker_ids.append(sent_speaker_ids)
      input_mask.append(sent_input_mask)
      seg_ids.append(sent_seg_ids)
    ...
    seg_ids = np.transpose(seg_ids)
    input_mask = np.transpose(input_mask)
    speaker_ids = np.transpose(speaker_ids)

    example_tensors = (input_ids, seg_ids, input_mask,  text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map)
  ...
  def truncate_example(self, input_ids, seg_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map, sentence_offset=None):
    ...
    seg_ids = seg_ids[:, sentence_offset:sentence_offset + max_training_sentences]
    ...
    return input_ids, seg_ids, input_mask, text_len, speaker_ids, genre, is_training,  gold_starts, gold_ends, cluster_ids, sentence_map
  ...
  def coarse_to_fine_pruning(self, top_span_emb, top_span_mention_scores, c):
    k = util_xlnet.shape(top_span_emb, 0)
    ...
    if self.config['use_prior']:
      ...
      distance_scores = util_xlnet.projection(tf.nn.dropout(tf.get_variable("antecedent_distance_emb", [10, self.config["feature_size"]], initializer=tf.truncated_normal_initializer(stddev=0.02)), self.dropout), 1, initializer=tf.truncated_normal_initializer(stddev=0.02)) #[10, 1]
    ...
    top_antecedents_mask = util_xlnet.batch_gather(antecedents_mask, top_antecedents) # [k, c]
    top_fast_antecedent_scores = util_xlnet.batch_gather(fast_antecedent_scores, top_antecedents) # [k, c]
    top_antecedent_offsets = util_xlnet.batch_gather(antecedent_offsets, top_antecedents) # [k, c]
    return top_antecedents, top_antecedents_mask, top_fast_antecedent_scores, top_antecedent_offsets


  def get_predictions_and_loss(self, input_ids, seg_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):

    run_config = xlnet.create_run_config(is_training=True, is_finetune=True, FLAGS=self.FLAGS)
    ...
    num_segs, seg_len = util_xlnet.shape(input_ids, 0), util_xlnet.shape(input_ids, 1)
      ...
      for i in range(self.config["coref_depth"]):
        with tf.variable_scope("coref_layer", reuse=(i > 0)):
          ...
          with tf.variable_scope("f"):
            f = tf.sigmoid(util_xlnet.projection(tf.concat([top_span_emb, attended_span_emb], 1), util_xlnet.shape(top_span_emb, -1))) # [k, emb]
  ...
  def get_mention_scores(self, span_emb, span_starts, span_ends):
      with tf.variable_scope("mention_scores"):
        span_scores = util_xlnet.ffnn(span_emb, self.config["ffnn_depth"], self.config["ffnn_size"], 1, self.dropout) # [k, 1]
      if self.config['use_prior']:
        ...
        with tf.variable_scope("width_scores"):
          width_scores =  util_xlnet.ffnn(span_width_emb, self.config["ffnn_depth"], self.config["ffnn_size"], 1, self.dropout) # [W, 1]
  ...
  def get_masked_mention_word_scores(self, encoded_doc, span_starts, span_ends):
      num_words = util_xlnet.shape(encoded_doc, 0) # T
      num_c = util_xlnet.shape(span_starts, 0) # NC
      ...
      with tf.variable_scope("mention_word_attn"):
        word_attn = tf.squeeze(util_xlnet.projection(encoded_doc, 1, initializer=tf.truncated_normal_initializer(stddev=0.02)), 1)
  ...
  def get_slow_antecedent_scores(self, top_span_emb, top_antecedents, top_antecedent_emb, top_antecedent_offsets, top_span_speaker_ids, genre_emb, segment_distance=None):
    k = util_xlnet.shape(top_span_emb, 0)
    c = util_xlnet.shape(top_antecedents, 1)
    ...
    with tf.variable_scope("slow_antecedent_scores"):
      slow_antecedent_scores = util_xlnet.ffnn(pair_emb, self.config["ffnn_depth"], self.config["ffnn_size"], 1, self.dropout) # [k, c, 1]
    slow_antecedent_scores = tf.squeeze(slow_antecedent_scores, 2) # [k, c]
    return slow_antecedent_scores # [k, c]

  def get_fast_antecedent_scores(self, top_span_emb):
    with tf.variable_scope("src_projection"):
      source_top_span_emb = tf.nn.dropout(util_xlnet.projection(top_span_emb, util_xlnet.shape(top_span_emb, -1)), self.dropout) # [k, emb]
  ...
  def flatten_emb_by_sentence(self, emb, text_len_mask):
    ...
    if emb_rank  == 2:
      ...
      flattened_emb = tf.reshape(emb, [num_sentences * max_sentence_length, util_xlnet.shape(emb, 2)])
    else:
      raise ValueError("Unsupported rank: {}".format(emb_rank))
    masked_out = tf.boolean_mask(flattened_emb, tf.reshape(text_len_mask, [num_sentences * max_sentence_length]))
    return masked_out
  ...
  def convert_mention(self, mention, comb_text, example):
    start = example['subtoken_map'][mention[0]]
    end = example['subtoken_map'][mention[1]] + 1
    nmention = (start, end)
  ...
  def evaluate(self, session, global_step=None, official_stdout=False, keys=None, eval_mode=False):
    ...
    for example_num, (tensorized_example, example) in enumerate(self.eval_data):
      ...
      if example_num % 10 == 0:
        ...
        if print_clusters:
          comb_text = [word for sentence in example['sentences'] for word in sentence]
          print('#### Example Clusters: ####')
          for cluster in example['clusters']:
            mapped = []
            for mention in cluster:
              mapped.append(self.convert_mention(mention, comb_text, example))
            print(mapped, end=",\n")
