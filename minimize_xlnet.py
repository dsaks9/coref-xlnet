  ...
  def finalize(self):
    ...
    for segment in self.segment_info:
      ...
      for i, tok_info in enumerate(segment):
        if tok_info is None and ((i == len(segment) - 1) or (i == len(segment) - 2)):
          speakers.append('[SPL]')
...
def split_into_segments(document_state, max_segment_len, constraints1, constraints2):
  ...
  while current < len(document_state.subtokens):
    ...
    document_state.segments.append(document_state.subtokens[current:end + 1] + ['[SEP]'] + ['[CLS]'])
    subtoken_map = document_state.subtoken_map[current : end + 1]
    document_state.segment_subtoken_map.append(subtoken_map + [subtoken_map[-1]] + [subtoken_map[-1]])
    info = document_state.info[current : end + 1]
    document_state.segment_info.append(info + [None] + [None])
    current = end + 1
...
def get_sentence_map(segments, sentence_end):
  ...
  for segment in segments:
    #sent_map.append(current) # No CLS in the beginning
    for i in range(len(segment) - 2):
      sent_map.append(current)
      current += int(sentence_end[sent_end_idx])
      sent_end_idx += 1
    sent_map.append(current)   # SEP at the end of the segment
    sent_map.append(current)   # CLS at the end of the segment
  return sent_map
...
def get_document(document_lines, tokenizer, language, segment_len):
  ...
  for line in document_lines[1]:
    ...
    if not sentence_end:
      ...
      processed_word = prepro_utils.preprocess_text(word)
      subtokens = prepro_utils.encode_pieces(tokenizer, processed_word)
...
def minimize_language(language, labels, stats, spiece_model_file, seg_len, input_dir, output_dir, do_lower_case):
  tokenizer = spm.SentencePieceProcessor()
  tokenizer.load(spiece_model_file)
...
if __name__ == "__main__":
  ...
  if not os.path.isdir(output_dir):
    os.mkdir(output_dir)
  for seg_len in [128, 256, 384, 512, 640, 768]:
    minimize_language("english", labels, stats, spiece_model_file, seg_len, input_dir, output_dir, do_lower_case)
